...
- Filters to families/registrants where `ever_filed_by_edgar_agents_llc_in_window = true`.

### 2) Family-level rollups

Python groups rows by family and computes:
- fund count (distinct registrants)
- distinct admin count
- distinct adviser count
- distinct non-EA filing agent count (from `agent_groups_used_in_window`, excluding EA aliases)
- total filings, QES filings, and QES share percent

### 3) Admin/adviser relationship map

Python explodes each row into admin × adviser pairs and rolls up:
- family count
- registrant count
- distinct non-EA filing agent count
- family list and non-EA agent list

### 4) Outputs

Generated by `src/ncen_family_exec_report_new.py`:
- PDF: `output/ncen_family_exec_report_new.pdf`
- Family CSV: `output/ncen_family_exec_report_new_families.csv`
- Admin/adviser CSV: `output/ncen_family_exec_report_new_admin_adviser.csv`
